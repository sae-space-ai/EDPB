...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Agent Status Header */}
      <div className="bg-gradient-to-r from-slate-900 via-red-900 to-rose-900 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-red-500/10 rounded-full -translate-y-1/2 translate-x-1/2"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-rose-500/10 rounded-full translate-y-1/2 -translate-x-1/2"></div>
        <div className="relative">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-red-200 text-lg">{status.role}</p>
              <p className="text-red-300/70 text-sm mt-1">{status.candidate_id} · {status.candidate}</p>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gradient-to-br from-red-50 to-rose-50 rounded-xl p-4 border border-red-200">
            <h3 className="font-bold text-red-800 mb-2">⚡ Execution Layer</h3>
            <p className="text-sm text-gray-600 mb-2">8 sub-agents + 8 security tools</p>
            <div className="text-xs text-red-600 font-medium">MEGAAGENTE CIBERSEGURIDAD</div>
          </div>
        </div>
      </div>
